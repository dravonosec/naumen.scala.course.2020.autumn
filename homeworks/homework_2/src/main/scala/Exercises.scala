object Exercises extends App {
  ...
  def primeFactor(number: Int): Seq[Int] = {
    var num = number
    var primeNubmers = Seq[Int]()
    var primeNumbersWithputCopy = Seq[Int]()
    var d = 2
    ...
    for (item <- primeNubmers)
        if (!primeNumbersWithputCopy.contains(item))
          primeNumbersWithputCopy :+= item

    primeNumbersWithputCopy
  }
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    val map = ballsArray.toSeq.sortBy(x => x._2._2 * x._2._1 * x._2._1 * x._2._1)
    for (ball <- map) yield ball._1
  }
}
